...
<script>
  ...
  export default {
    props: ['projectSkill', 'skills', 'project'],

    data() {
      return {
        skill_id: '',
        percentage: '',
        editing: !this.projectSkill.id,
        hidden: false
      }
    },
    ...
    methods: {
      ...
      storeSkill() {
        projectSkillService.store(this.project.id, JSON.stringify({
          id: this.selected.id,
          percentage: this.percentage
        }))
          .then(({data}) => {
            this.projectSkill.id = data.id;
          }, (response) => {
            this.reset();
          });
      },
      updateSkill() {
        projectSkillService.update(this.project.id, this.projectSkill.id, JSON.stringify({
          id: this.selected.id,
          percentage: this.percentage
        }))
          .then(({data}) => {
      ...
      deleteSkill() {
        projectSkillService.delete(this.project.id, this.projectSkill.id)
          .then(({data}) => {
            this.hidden = true;
          }, (response) => {
            this.reset();
      ...
      reset() {
        this.skill_id = this.projectSkill.id;
        this.percentage = this.projectSkill.percentage;
      }
    }
  }
</script>